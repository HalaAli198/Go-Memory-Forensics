...
Per target process the plugin:
  1. Locates the in-memory ELF image and parses its program headers.
...
Usage:
    python3 vol.py -f <image> linux.go_strings.Go_Strings --pid <PID>
"""
import logging
...
class Go_Strings(interfaces.plugins.PluginInterface):
    ...
    def _find_pclntab(self, segments: List[Dict]) -> Optional[Dict]:
        ...
        print(f"\n[*] Scanning for pclntab in RODATA: {hex(start)}-{hex(end)}")
    ...
    def _find_mheap_global(self, sections: List[Dict]) -> Optional[Dict]:
      ...
      if not scan_regions:
        for idx, seg in enumerate(segments):
            if seg.get("p_flags_str") == "RW-":
                scan_regions.append((f"rw_seg_{idx}", seg["runtime_vaddr"], seg["runtime_end"]))
    ...
    def _is_go_heap_pointer(self, ptr):
      ...
      if ptr < 0xc000000000:  # Go amd64 arena base  
        return False
    ...
    def _generator(self) -> Generator[Tuple[int, Tuple], None, None]:
     ...
     try: 
      ...
      for task in tasks:
           ...
           if spans:
              ...
              if heap_objects:
                    ...
                    for s in heap_strings:
                        ...
                        print(f"{hex(s['struct_addr']):<20} {hex(s['data_addr']):<20} {s['length']:<10} {quoted_value:<60}"
                        f"{s['struct_location']:<20} {s['data_location']:<20}")
        ...
        vollog.error("Plugin crashed: %s", e, exc_info=True)
        import traceback
        traceback.print_exc()
     return
     yield
