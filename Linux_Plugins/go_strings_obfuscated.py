...
Obfuscation resilience (Garble):
  ...
  - Stripped Go version string.  Garble removes the "go1.X.Y" literal
    from rodata, so the normal version-extraction scan returns "unknown".
  ...
  - Incorrect mspan layout selection.  Because the inferred Go version may
    ...
    selecting the profile with the highest validation score.  This avoids
    misreading the state byte and classifying all spans as dead.

Usage:
    python3 vol.py -f <image> linux.go_strings_obfuscated.Go_Strings --pid <PID>
"""
import logging
...
class Go_Strings(interfaces.plugins.PluginInterface):
    ...
    def _parse_mspan(self, span_addr: int, stats: Optional[Dict] = None) -> Optional[Dict]:
      ...
      try:
        ...
        # Common fields (same offset in all versions)
        ...
        # For Go 1.24+, nelems might need calculation
    ...
    def _generator(self) -> Generator[Tuple[int, Tuple], None, None]:
     ...
     try: 
      ...
      for task in tasks:
        ...
        # Infer Go version from pclntab if RODATA scan failed (e.g. Garble'd binary)
