...
Pipeline:
    1. Locate ELF in process VMAs; handle ET_EXEC and ET_DYN (PIE).
    ...
    6. Resolve source files via pcfile→cutab→filetab; supplement from
       page-cache ELF for stripped binaries.
...
Architecture:
    x86-64 Linux (Intel64). 32-bit support is partial.
...
Usage:
    python3 vol.py -f <image> linux.go_functions.Go_Functions --pid <PID>
...
class Go_Functions(interfaces.plugins.PluginInterface):
      ...
      for magic_bytes, version_info in self.GO_MAGICS.items():
        ...
        # =====================================================
            ...
            for i in range(nfunc):
                ...
                # _func struct is at pclntable + funcoff
    ...
    def _extract_elf_from_pagecache(self, inode) -> bytes:
      """Extract ELF bytes from page cache using InodePages logic."""
      try:
    ...
    def  _load_external_func_db(self, go_version: str) -> Dict:
      ...
      db_file = f"/home/hala/file_func_params_extractor/go_func_lines_v1255.json"  #  the path depdens of the analyzed Go version
    ...
    def _print_ida_style_call_analysis(self, caller_instructions: List[Dict], 
                                    ...
                                    recovered_args: List[Dict],  func_lookup: Dict[int, Dict] = None):
      """
       Print IDA-style annotated disassembly around a CALL site.
      """
      print(f"\n{'─'*80}")
    ...
    def _get_instruction_annotation(self, insn: Dict, 
      ...
      """
      Generate semantic annotation for a single instruction.

      Returns annotation string or None. Handles:
        - LEA [rip+X]: reads string at target, shows content
        - LEA [rsp+X]: shows &stack[offset]
        - MOV [rsp+X], imm: shows = decimal_value
        - CALL 0xAddr: resolves to function name via func_lookup
        - XOR reg,reg: shows = 0
      """
    ...
    def _get_callee_parameters(self, caller_pc, caller_name, callee_pc, type_methods, called_func_info, _func_functions_new, caller_instructions=None):
        """
        Dispatch argument recovery based on callee category.

        Routes to the appropriate recovery method:
        - Type methods:  _get_callee_parameters_from_type_method
        - Stdlib/runtime: _get_callee_parameters_from_external_info
        - Application:   _get_callee_parameters_static (ArgInfo) or
                       _get_callee_parameters_from_argsmap (ArgsPointerMaps)
        - Third-party:   _lookup_third_party_function (metadata only)

        After recovery, prints IDA-style annotated disassembly via
        _print_ida_style_call_analysis.
        """
    ...
    def _use_register_abi(self) -> bool:
      """
      Determine if the binary uses register-based ABI (Go 1.17+) or stack-based ABI (Go 1.16 and earlier).
      """
      major, minor, _ = self.go_version_tuple
      # Register ABI was introduced in Go 1.17
    ...
    def _recover_argument_from_slots(self, arg_info: Dict, slot_values: Dict) -> Dict:
      ...
      if inferred_type == 'string':
        ...
        if ptr_value is not None and len_value is not None:
            if 0 < len_value < 1000000 and ptr_value > 0x10000:
    ...
    def _analyze_instructions_before_call_stack_abi(self, instructions: List[Dict], call_index: int, lookback: int = 100) -> Dict[int, Dict]:
      """
      Backward instruction walk for stack ABI (Go ≤1.16).

      Walks backward from CALL, recording MOV [rsp+N] stores and tracking
      register values needed to resolve them.

      IMPORTANT: Does NOT stop at intermediate CALLs because Go ≤1.16 passes
      arguments on the stack, and stack slots persist across calls. Only
      registers are marked clobbered at each intermediate CALL; the scan
      continues to find earlier stack stores.

      Returns:
        stack_state: {stack_offset: {type: 'resolved'|'runtime', value}}
      """
      stack_state = {}
      register_state = {}
    ...
    def _analyze_instructions_before_call(self, instructions: List[Dict], call_index: int, 
      ...
      """
      Backward instruction walk for register ABI (Go 1.17+).

      Walks backward from CALL, recording the most recent assignment to each
      register and stack offset. Stops at the first intermediate CALL because
      Go 1.17+ passes arguments in caller-saved registers (RAX, RBX, RCX, etc.)
      which are clobbered by any intervening call — instructions before that
      call were setting up ITS arguments, not ours.

      After the walk, resolves pending stack stores whose source registers
      were found later in the backward scan, and cross-references stack_load
      register entries against resolved stack slots.

      Returns:
        register_state: {reg_name: {type, value/target/src_reg/offset}}
        stack_state:    {stack_offset: {type: 'resolved'|'runtime', value}}
      """
      register_state = {}
      stack_state = {}
