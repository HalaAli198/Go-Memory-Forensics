...
go_goroutines.py — Volatility 3 plugin: Go goroutine stack trace and
                   argument recovery from Windows memory dumps.
...
Pipeline: PE base (PEB/VADs) → PE sections → Go version → pclntab →
moduledata → cached PE via dumpfiles → functions/types/itabs → allgs →
...
Dependencies:
    - pefile library (PE parsing for cached binary)
...
from volatility3.plugins.windows import dumpfiles
...
class Go_Goroutines(interfaces.plugins.PluginInterface):
    ...
    def _load_external_func_db(self, go_version: str) -> Dict:
      ...
      db_file = f"/home/hala/func_lines_extractor/go_func_lines_v115.json"  # Update this path
    ...
    def _load_heap_addresses(self):
        filepath = "/home/hala/volatility3/heap_strings_pid_1100.json"
    ...
    def _validate_moduledata_go115(self, address: int, pclntab_addr: int, ptrSize: int, sections) -> dict:
      ...
      try:
        ...
        print(f"\nRaw moduledata bytes at {hex(address)}:")
        for i in range(0, 0x50, 8):
            chunk = data[i:i+8]
            val = int.from_bytes(chunk, 'little')
            print(f"  +{hex(i)}: {chunk.hex()} -> {hex(val)}")
        
        print(f"\n Parsed slices:")
        print(f"  pclntable @ +0x00: ptr={hex(pclntable_ptr)}, len={pclntable_len}, cap={pclntable_cap}")
        print(f"  ftab      @ +0x18: ptr={hex(ftab_ptr)}, len={ftab_len}, cap={ftab_cap}")
        print(f"  filetab   @ +0x30: ptr={hex(filetab_ptr)}, len={filetab_len}, cap={filetab_cap}")
        # === END DEBUG BLOCK ===
        return {
            'address': address,
            ...
            'filetab': {'ptr': filetab_ptr, 'len': filetab_len, 'cap': filetab_cap},
            'pctab': {'ptr': pclntable_ptr, 'len': pclntable_len, 'cap': pclntable_cap},
            'cutab': {'ptr': 0, 'len': 0, 'cap': 0},
            'minpc': minpc,
            'maxpc': maxpc,
            'text': text,
            'etext': etext,
    ...
    def _extract_functions(self, layer_name: str, pclntab: Dict, moduledata: Dict) -> Generator[Dict, None, None]:
      ...
      is_go_118_plus = (major == 1 and minor >= 18 and minor < 24)
      is_go_116_117 = (major == 1 and minor >= 16 and minor < 18)
      ...
      print(f"[*] _func struct base size: {func_struct_base_size} bytes (Go {major}.{minor})")
        ...
        try:
            ...
            if nfuncdata > 0:
               ...
               if len(funcdata_ptrs) > self.FUNCDATA_ArgsPointerMaps and funcdata_ptrs[self.FUNCDATA_ArgsPointerMaps] != 0:
                  argsmap_data = self._parse_args_from_stackmap(layer,funcdata_ptrs[self.FUNCDATA_ArgsPointerMaps], ptrSize)
    ...
    def _parse_args_from_stackmap(self, layer, stackmap_addr: int, ptrSize: int) -> Optional[Dict]:
      """Parse ArgsPointerMaps - now accepts ptrSize parameter."""
      try:
        if stackmap_addr == 0:
            return None
        ...
        if n < 0 or n > 1000 or nbit < 0 or nbit > 100:
            print(f"stackmap validation failed: n={n}, nbit={nbit}")
            return None
        ...
        vollog.debug(f"Error parsing stackmap: {e}")
        return None
    ...
    def _find_allgs_via_scan(self, layer_name: str, sections: List[Dict], ptrSize: int) -> Optional[Dict]:
      """
      ...
      rw_section = None
      for sect in sections:
        if sect["p_flags_str"] == "RW-" or sect["name"] in [".data", ".bss"]:
            rw_section = sect
            break
    
      if not rw_section:
        print("[!] No RW section found")
        return None
    
      start = rw_section["runtime_vaddr"]
      end = rw_section["runtime_end"]
      print(f"[*] Scanning RW section '{rw_section['name']}': {hex(start)}-{hex(end)}")
      layer = self.context.layers[layer_name]
      slice_size = ptrSize * 3
      ...
      try:
       ...
       # print(f"TYPE: '{type_name}' @ {hex(type_addr)}")
        ...
        for i in range(mcount):
            ...
            name_offset = int.from_bytes(method_data[0:4], 'little', signed=True)
            #print(f"\n  METHOD {i}:")
            #print(f"    Raw bytes: {method_data.hex()}")
            #print(f"  method_offset: {hex(method_offset)}")
            #print(f"    name_offset: {name_offset} (0x{name_offset:x})")
            if len(method_data) < method_size:
                break
    ...
    def _resolve_filename_from_pe(self, pe_bytes: bytes, pctab_file: int, filetab_file: int,
      ...
      """Resolve source filename from cached PE bytes (Go 1.16+)."""
      try:
        # Decode pcfile to get file index
        pcfile_data = pe_bytes[pctab_file + pcfile_off:]
        if not pcfile_data:
            return "<unknown>"
        
        file_index = self._pcvalue_from_bytes(pcfile_data, func_pc, func_pc)
        if file_index is None or file_index < 0:
            return "<unknown>"
        
        # Actual cutab index = cuOffset + file_index
        actual_idx = cuOffset + file_index
        
        # Read filetab offset from cutab
        cutab_entry = cutab_file + (actual_idx * 4)
        if cutab_entry + 4 > len(pe_bytes):
            return "<unknown>"
        
        filetab_offset = int.from_bytes(pe_bytes[cutab_entry:cutab_entry + 4], 'little')
        
        # Read filename string from filetab
        name_pos = filetab_file + filetab_offset
        if name_pos >= len(pe_bytes):
            return "<unknown>"
        
        end = pe_bytes.find(b'\x00', name_pos, name_pos + 512)
        if end == -1:
            end = name_pos + 512
        
        return pe_bytes[name_pos:end].decode('utf-8', errors='replace')
      except:
        return "<unknown>"


    def _resolve_filename_from_pe_go115(self, pe_bytes: bytes, pclntab_file: int,
                                     pcfile_off: int, func_pc: int) -> str:
       """Resolve source filename from cached PE bytes (Go 1.2-1.15)."""
       try:
          # Decode pcfile to get file index
          pcfile_data = pe_bytes[pclntab_file + pcfile_off:]
          if not pcfile_data:
            return "<unknown>"
          ...
          # filetab starts after ftab in the pclntab
          # Read file offset from filetab
          ptrSize = self.pclntab.get('ptrSize', 8)
          nfunc = self.pclntab.get('nfunc', 0)
        
          # ftab starts at pclntab + header_size
          header_size = 8 + ptrSize  # magic(4)+pad(2)+minLC(1)+ptrSize(1)+nfunc(ptrSize)
          ftab_size = (nfunc + 1) * (2 * ptrSize)
          filetab_start = pclntab_file + header_size + ftab_size
        
          filetab_entry = filetab_start + (file_index * 4)
          if filetab_entry + 4 > len(pe_bytes):
            return "<unknown>"
        
          name_offset = int.from_bytes(pe_bytes[filetab_entry:filetab_entry + 4], 'little')
          name_pos = pclntab_file + name_offset
    ...
    def _pcvalue_from_bytes(self, pctab_data: bytes, target_pc: int, entry_pc: int) -> Optional[int]:
      """Decode pcvalue from raw bytes (for cached file parsing, not live memory)."""
      if not pctab_data:
        return None
      ...
      while idx < len(pctab_data) and idx < 1024:
        val_delta, consumed = self._read_varint_zigzag(pctab_data[idx:])
        if consumed == 0:
            break
        idx += consumed
    ...
    def _parse_map_old( self,map_ptr: int, key_type_ptr: int, elem_type_ptr: int, types_dict: Dict, itabs_dict: Dict, _func_functions: Dict, depth:int,
     max_depth:int) -> Dict:
      try:
        ...
        print(f"[MAP DEBUG] Old hmap error: {e}")
        return {'error': str(e)}
    ...
    def _lookup_func_name_from_external(self, filename: str, line_number: int, 
      ...
      for func in funcs:
        ...
        if entry_line <= line_number and entry_line > best_entry_line:
            ...
            containing_func = func['func_name']
                     ...
                     if classify_go_filepath(filename).get('category') in ["runtime_core", "runtime_internal", "stdlib_internal", "stdlib_public"]:
                       ...
                       if args: 
                          if arginfo_data:  
                             ...
                             # Store arguments under frame_key
                             ...
                             for arginfo_idx, arg in enumerate(arginfo_args):
                                 ...
                                 if param_info is None:
                                    ...
                                    param_name = f"arg{arginfo_idx}"  # Generic name
                                    print(f" Arg {param_name}: {param_info['value']}")
                                 
                                    self.non_type_method_arguments[func_pc][frame_key][arginfo_idx] = {
                                   'param_name': param_name,
                                   'param_address': value_addr,
                                   'param_type': param_info.get('type_name', ''),
    ...
    def _get_non_type_argument_info(self, arg,  Pointer_offsets: List[int], arg_base: int, itabs_dict: Dict[int, Dict], 
      ...
      # Aggregate argument (multiple fields)
          ...
          if num_fields == 2 and field_sizes == [8, 8]:
            ...
            # print(f"field0_is_ptr :{field0_is_ptr}")
             #print(f"field1_is_ptr :{field1_is_ptr}")
          ...
          if num_fields == 3 and field_sizes == [8, 8, 8]:
            ...
            if field0_is_ptr:
                ...
                if slice_ptr in self.data_to_type_map:
                   #print(f"    ✓ Slice found in METHOD map!")
    ...
    def _get_file_line_for_pc(self, func_info: Dict, target_pc: int) -> Tuple[str, int]:
      try:
        ...
        # Get line number from pcln table
        line_number = 0
        if pcln_offset > 0:
            pcln_addr = pctab_base + pcln_offset
            pcln_data = layer.read(pcln_addr, 1024, pad=True)
            line_number = self._pcvalue(pcln_data, target_pc, entry_pc,
        ...
        if file_index is not None and file_index >= 0:
            if is_go_116_plus:
                # Go 1.16+: file_index needs cuOffset adjustment
                filename = self._resolve_filename_go116(file_index, cuOffset)
            else:
                # Go 1.2-1.15: file_index is direct index into filetab
                filename = self._resolve_filename(file_index)
        
        return (filename, line_number if line_number else 0)
    ...
    def _resolve_filename(self, file_index: int) -> str:
      """Resolve filename from file index."""
      try:
        if file_index < 0:
            return "<negative_idx>"
        ...
        is_go_116_plus = (major == 1 and minor >= 16) 
        ...
        if is_go_116_plus:
            ...
            pclntable_ptr = self.moduledata.get('pclntable', {}).get('ptr', 0)
            pclntable_len = self.moduledata.get('pclntable', {}).get('len', 0)
            filetab_ptr = self.moduledata.get('filetab', {}).get('ptr', 0)
            filetab_len = self.moduledata.get('filetab', {}).get('len', 0)
            if filetab_ptr == 0 or pclntable_ptr == 0:
                return "<no_filetab>"
            
            if file_index >= filetab_len:
                return f"<idx_oob:{file_index}/{filetab_len}>"
            
            filetab_entry_addr = filetab_ptr + (file_index * 4)
            offset_data = layer.read(filetab_entry_addr, 4, pad=True)
            pclntable_offset = int.from_bytes(offset_data, 'little')
            
            # Validate offset is within pclntable bounds
            pclntable_len = self.moduledata.get('pclntable', {}).get('len', 0)
            if pclntable_offset >= pclntable_len:
                return f"<offset_oob:{pclntable_offset}>"
            
            name_addr = pclntable_ptr + pclntable_offset
            filename = self._read_cstring(name_addr, 512)
            
            # If filename is empty, it's likely a stripped runtime file
            if not filename:
                return "<runtime>"  # Better placeholder than <empty>
     ...
     try: 
      ...
      for proc in pslist.PsList.list_processes(context=self.context, kernel_module_name=self.config["kernel"], filter_func=filter_func):
        ...
        if cached_pe_bytes:
           ...
           print("[!] Could not get cached binary")
        ...
        print(f"\nType Distribution:")
        for kind_str, count in sorted(type_counts.items(), key=lambda x: x[1], reverse=True):
            print(f"  {kind_str}: {count}")
        ...
        # Get the goroutines.  
        allgs = self._find_allgs_via_scan(self.layer_name, sections, pclntab["ptrSize"])
        if not allgs:
