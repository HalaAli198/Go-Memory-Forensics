"""
go_strings: type-aware recovery of Go runtime strings from process memory.
...
Per target process the plugin:
  1. Locates the in-memory PE image and parses its program headers.
  2. Detects the Go toolchain version from the rodata segment.
...
Usage:
    python3 vol.py -f <image> windows.go_strings.Go_Strings --pid <PID>
"""
...
class Go_Strings(interfaces.plugins.PluginInterface):
    ...
    def _find_pclntab(self, sections: List[Dict]) -> Optional[Dict]:
        """Find Go pclntab by scanning for magic bytes in RODATA."""
        rodata_section  = None
        ...
        if not rodata_section:
           # Fallback: search all readable sections
           for sect in sections:
        ...
        print(f"\n[*] Scanning for pclntab in {rodata_section['name']}: {hex(start)}-{hex(end)}")
    ...
    def _quick_validate_mspan(self, span_ptr: int, layer) -> bool:
      """
      Quick validation of mspan structure.
      Checks key fields without full parsing.
      """
      try:
        major, minor, _ = self.go_version_tuple
        
        # Read mspan header (first 0x80 bytes should be enough)
        data = layer.read(span_ptr, 0x80, pad=True)
        
        # Common across versions: startAddr at +0x18, npages at +0x20
        startAddr = int.from_bytes(data[0x18:0x20], 'little')
        npages = int.from_bytes(data[0x20:0x28], 'little')
        
        # Validate startAddr - should be in Go heap range (0xc0...)
        if not (0xc000000000 <= startAddr < 0xd000000000):
            # Also allow other valid memory ranges
            if startAddr < 0x1000:
                return False
        ...
        # Check state field - position varies by version
        if major == 1 and minor >= 25:
            state = data[0x63]
        elif major == 1 and minor >= 20 and  minor < 25:
            state = data[0x6C]
        else:
            state = data[0x63]
        
        # State should be 0 (dead), 1 (in use), or 2 (manual)
        if state > 2:
            return False
    ...
    def _parse_mspan(self, span_addr: int, stats: Optional[Dict] = None) -> Optional[Dict]:
      """Parse a single mspan structure (64-bit only)."""
      ...
      try:
        ...
        if major == 1 and minor >= 24 and minor <= 25:
           allocBits_ptr = self._read_pointer(span_addr + 0x40)
           allocCount = self._read_uint16(span_addr + 0x60)
           spanclass = self._read_uint8(span_addr + 0x62)
           state = self._read_uint8(span_addr + 0x63)
           elemsize = self._read_pointer(span_addr + 0x68)
           if elemsize > 0 :
                nelems = (npages * 8192) // elemsize
           else:
                nelems = 0
        elif major == 1 and minor >= 20 and minor < 24:
            nelems = self._read_pointer(span_addr + 0x38)
            allocBits_ptr = self._read_pointer(span_addr + 0x48)
            allocCount = self._read_uint16(span_addr + 0x68)
            spanclass = self._read_uint8(span_addr + 0x6A)
            state = self._read_uint8(span_addr + 0x6C)
            elemsize = self._read_pointer(span_addr + 0x70)
        
        elif major == 1 and minor >= 15 and minor < 20:  
           nelems = self._read_pointer(span_addr + 0x38)
           allocBits_ptr = self._read_pointer(span_addr + 0x48)
           allocCount    = self._read_uint16(span_addr + 0x60)
           spanclass     = self._read_uint8(span_addr + 0x62)
           state         = self._read_uint8(span_addr + 0x63)
           elemsize = self._read_pointer(span_addr + 0x68)
           #print(f"hala nelems:{nelems}, allocBits_ptr:{hex(allocBits_ptr)}, allocCount: {allocCount}, spanclass: {spanclass}, state: {state}, elemsize: {elemsize}")
           if elemsize ==16 or elemsize ==16:
              print(f"hala span_addr :{hex(span_addr)}, nelems:{nelems}, allocBits_ptr:{hex(allocBits_ptr)}, allocCount: {allocCount}, spanclass: {spanclass}, state: {state}, elemsize: {elemsize}")
        
       
        # State validation
        if state == 0:  # mSpanDead
            if stats:
                stats['dead'] += 1
            return None
        
        if state == 2:  # mSpanManual (stacks, not heap)
            if stats:
                stats['manual'] += 1
            return None
        
        if state != 1:  # Only accept mSpanInUse
            if stats:
                stats['invalid'] += 1
            return None
        
        # Field validation
        if startAddr == 0 or elemsize == 0 or nelems == 0:
            if stats:
                stats['invalid'] += 1
            return None
        
        # Size validation
        if elemsize > 0x20000000:  # 512MB max
            if stats:
                stats['invalid'] += 1
            return None
      ...
      for i in range(allspans_len):
        ...
        try:
            ...
            if span_ptr == 0:
                stats['null_ptr'] += 1
                continue
            if i < 10 and span_ptr != 0:
                self._dump_mspan_raw(span_ptr)
            # Parse the mspan WITH stats tracking
            span_info = self._parse_mspan(span_ptr, stats)  # ← Pass stats
            
            if span_info:  # Already filtered to state=1
