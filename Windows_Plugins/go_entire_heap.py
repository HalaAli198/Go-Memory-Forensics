...
image of a running Go (golang) process on Windows.  Rather than scanning
...
Per target process the plugin:
  ...
  1. Locates the in-memory PE image and parses its section table.
...
Usage:
    ...
    python3 vol.py -f <image> windows.go_entire_heap.Go_Entire_Heap --pid <PID>
"""
import logging
...
class Go_Entire_Heap(interfaces.plugins.PluginInterface):
    ...
    def _validate_moduledata(self, address: int, pclntab_addr: int, ptrSize: int, go_version: str, sections: List[Dict]) -> Optional[Dict]:
      ...
      try:
        ...
        if is_go_116_117:
            ...
            print(f"\n[DEBUG] ========== Go 1.16-1.17 MODULEDATA ==========")
            print(f"[DEBUG] Go Version: {go_version}")
        ...
        mheap_ptr = 0
        print(f"\n[DEBUG] ========== SLICE POINTER ANALYSIS ==========")
        print(f"[DEBUG] Go Version: {go_version}")
        print(f"[DEBUG] Is Go 1.18+: {is_go_118_plus}")
    ...
    def _scan_for_allspans_slice(self, start: int, end: int, layer) -> Optional[Dict]:
      ...
      print(f"[DEBUG] Scanning range: {hex(start)} - {hex(end)}")
        ...
        try:
            ...
            for offset in range(0, len(data) - 24, 8):
                ...
                if self._validate_allspans_array(slice_ptr, slice_len, layer):
                    ...
                    # DEBUG: Dump first few span pointers
                    #self._debug_dump_span_pointers(slice_ptr, min(slice_len, 5), layer)
            ...
            print(f"[DEBUG] Exception at {hex(current)}: {e}")
            current += chunk_size
    
      print(f"[DEBUG] Total candidates found: {candidates_found}")
      return None
    ...
    def _debug_dump_span_pointers(self, array_ptr: int, count: int, layer):
      ...
      try:
        print(f"  [DEBUG] Reading {count} span pointers from {hex(array_ptr)}:")
        ...
        print(f"  [DEBUG] Error dumping span pointers: {e}")
    ...
    def _generator(self) -> Generator[Tuple[int, Tuple], None, None]:
     try: 
      ...
      for proc in pslist.PsList.list_processes(context=self.context, kernel_module_name=self.config["kernel"], filter_func=filter_func):
        ...
        print(f"[DEBUG] typelinks: ptr={hex(moduledata['typelinks']['ptr'])}, len={moduledata['typelinks']['len']}")
        print(f"[DEBUG] types section: {hex(moduledata['types'])}-{hex(moduledata['etypes'])}")
        print("\n" + "=" * 170)
